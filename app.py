...
def total_books():
    if 'loggedin' in session:
        ...
        return render_template("total_books.html", books=books)
    return redirect(url_for('login'))
...
@app.route("/returned_books", methods=['GET'])
def returned_books():
    if 'loggedin' in session:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute("SELECT COUNT(*) AS returned_count FROM issued_book WHERE status='returned'")  
        result = cursor.fetchone()
        returned_count = result['returned_count']
        return render_template("returned_books.html", returned_count=returned_count)
    return redirect(url_for('login'))
